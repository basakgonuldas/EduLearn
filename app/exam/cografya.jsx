...
import geographyExamQuestions from '../../data/exam/cografya';
import { Ionicons } from '@expo/vector-icons';
import {useRouter} from 'expo-router'

export default function Geography({ navigation }) {
  ...
  const currentQuestion = geographyExamQuestions [currentIndex];
  ...
  const handleNext = () => {
    if (currentIndex + 1 <  geographyExamQuestions.length) {
  ...
  if (showResult) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.score}>Skor: {score} / {geographyExamQuestions.length}</Text>
      </View>
    );
  }
