...
import kimyaExamQuestions  from '../../data/exam/kimya';
import { Ionicons } from '@expo/vector-icons';
import {useRouter} from 'expo-router'

export default function Kimya({ navigation }) {
  ...
  const currentQuestion =kimyaExamQuestions [currentIndex];
  ...
  const handleNext = () => {
    if (currentIndex + 1 < kimyaExamQuestions.length) {
  ...
  if (showResult) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.score}>Skor: {score} / {kimyaExamQuestions .length}</Text>
      </View>
    );
  }
