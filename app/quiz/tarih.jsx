...
import tarihQuestions from '../../data/quiz/tarihdata';
...
const HistoryApp = () => {
  ...
  if (currentIndex >=tarihQuestions.length) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>🎉 Oyun Bitti!</Text>
        <Text style={styles.scoreText}>Puanın: {score} / {tarihQuestions.length}</Text>
  ...
  const { question, answer } = tarihQuestions[currentIndex];

  return (
    <View style={styles.container}>
      <Text style={styles.title}>🧠 Tarih Flashcard Oyunu</Text>
      <Text style={styles.progressText}>Soru {currentIndex + 1} / {tarihQuestions.length}</Text>
