...
import biyolojiQuestions from '../../data/quiz/biyolojidata';
...
const BiyologyApp = () => {
  ...
  if (currentIndex >= biyolojiQuestions.length) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>🎉 Oyun Bitti!</Text>
        <Text style={styles.scoreText}>Puanın: {score} / {biyolojiQuestions.length}</Text>
  ...
  const { question, answer } = biyolojiQuestions[currentIndex];

  return (
    <View style={styles.container}>
      <Text style={styles.title}>🧠 Biyoloji Flashcard Oyunu</Text>
      <Text style={styles.progressText}>Soru {currentIndex + 1} / {biyolojiQuestions.length}</Text>
